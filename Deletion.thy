...
locale Deletion =
  ...
  l: injective_morphism K L l
  for K G L g l +
  assumes
    dang_src: \<open>e \<in> E\<^bsub>G\<^esub> - \<^bsub>g\<^esub>\<^sub>E ` (E\<^bsub>L\<^esub> - \<^bsub>l\<^esub>\<^sub>E ` E\<^bsub>K\<^esub>) \<Longrightarrow> s\<^bsub>G\<^esub> e \<notin> \<^bsub>g\<^esub>\<^sub>V ` (V\<^bsub>L\<^esub> - \<^bsub>l\<^esub>\<^sub>V ` V\<^bsub>K\<^esub>)\<close> and
    dang_trg: \<open>e \<in> E\<^bsub>G\<^esub> - \<^bsub>g\<^esub>\<^sub>E ` (E\<^bsub>L\<^esub> - \<^bsub>l\<^esub>\<^sub>E ` E\<^bsub>K\<^esub>) \<Longrightarrow> t\<^bsub>G\<^esub> e \<notin> \<^bsub>g\<^esub>\<^sub>V ` (V\<^bsub>L\<^esub> - \<^bsub>l\<^esub>\<^sub>V ` V\<^bsub>K\<^esub>)\<close>
begin


abbreviation V where \<open>V \<equiv> V\<^bsub>G\<^esub> - \<^bsub>g\<^esub>\<^sub>V ` (V\<^bsub>L\<^esub> - \<^bsub>l\<^esub>\<^sub>V ` V\<^bsub>K\<^esub>)\<close>
abbreviation E where \<open>E \<equiv> E\<^bsub>G\<^esub> - \<^bsub>g\<^esub>\<^sub>E ` (E\<^bsub>L\<^esub> - \<^bsub>l\<^esub>\<^sub>E ` E\<^bsub>K\<^esub>)\<close>
...
\<open>d \<equiv> g \<circ>\<^sub>\<rightarrow> l\<close>
...
next
  show \<open>\<^bsub>d\<^esub>\<^sub>V (s\<^bsub>K\<^esub> e) = s\<^bsub>D\<^esub> (\<^bsub>d\<^esub>\<^sub>E e)\<close> if \<open>e \<in> E\<^bsub>K\<^esub>\<close> for e
    using that wf_morph_comp[of K L l G g]
...
abbreviation p where \<open>p \<equiv> idM\<close>
interpretation inj_d: injective_morphism D G p
proof
  show \<open>\<^bsub>idM\<^esub>\<^sub>E e \<in> E\<^bsub>G\<^esub>\<close> if \<open>e \<in> E\<^bsub>D\<^esub>\<close> for e
    using that by simp
next
  show \<open>\<^bsub>idM\<^esub>\<^sub>V v \<in> V\<^bsub>G\<^esub>\<close> if \<open>v \<in> V\<^bsub>D\<^esub>\<close> for v
    using that by simp
next
  show \<open>\<^bsub>idM\<^esub>\<^sub>V (s\<^bsub>D\<^esub> e) = s\<^bsub>G\<^esub> (\<^bsub>idM\<^esub>\<^sub>E e)\<close> if \<open>e \<in> E\<^bsub>D\<^esub>\<close> for e
    using that by simp
next
  show \<open>\<^bsub>idM\<^esub>\<^sub>V (t\<^bsub>D\<^esub> e) = t\<^bsub>G\<^esub> (\<^bsub>idM\<^esub>\<^sub>E e)\<close> if \<open>e \<in> E\<^bsub>D\<^esub>\<close> for e
    using that by simp
next
  show \<open>l\<^bsub>D\<^esub> v = l\<^bsub>G\<^esub> (\<^bsub>idM\<^esub>\<^sub>V v)\<close> if \<open>v \<in> V\<^bsub>D\<^esub>\<close> for v
    using that by simp
next
  show \<open>m\<^bsub>D\<^esub> e = m\<^bsub>G\<^esub> (\<^bsub>idM\<^esub>\<^sub>E e)\<close> if \<open>e \<in> E\<^bsub>D\<^esub>\<close> for e
    using that by simp
next
  show \<open>inj_on \<^bsub>idM\<^esub>\<^sub>V V\<^bsub>D\<^esub>\<close>
    by simp
next
  show \<open>inj_on \<^bsub>idM\<^esub>\<^sub>E E\<^bsub>D\<^esub>\<close>
    by simp
qed
...
interpretation pushout_diagram K L D G l d g p
proof
  show \<open>\<^bsub>d\<^esub>\<^sub>V v = \<^bsub>idM \<circ>\<^sub>\<rightarrow> d\<^esub>\<^sub>V v\<close> if \<open>v \<in> V\<^bsub>K\<^esub>\<close> for v
    by (simp add: morph_comp_def that)
next
  show \<open>\<^bsub>d\<^esub>\<^sub>E e = \<^bsub>idM \<circ>\<^sub>\<rightarrow> d\<^esub>\<^sub>E e\<close> if \<open>e \<in> E\<^bsub>K\<^esub>\<close> for e
    by (simp add: morph_comp_def that)
next
  show \<open>Ex1M (\<lambda>xa. morphism G D' xa 
    ...
    \<and> (\<forall>v\<in>V\<^bsub>D\<^esub>. \<^bsub>xa \<circ>\<^sub>\<rightarrow> idM\<^esub>\<^sub>V v = \<^bsub>y\<^esub>\<^sub>V v) \<and> (\<forall>e\<in>E\<^bsub>D\<^esub>. \<^bsub>xa \<circ>\<^sub>\<rightarrow> idM\<^esub>\<^sub>E e = \<^bsub>y\<^esub>\<^sub>E e)) G\<close>
    if \<open>graph D'\<close> \<open>morphism L D' x\<close> \<open>morphism D D' y\<close>
      \<open>\<forall>v\<in>V\<^bsub>K\<^esub>. \<^bsub>x \<circ>\<^sub>\<rightarrow> l\<^esub>\<^sub>V v = \<^bsub>y \<circ>\<^sub>\<rightarrow> d\<^esub>\<^sub>V v\<close>
      \<open>\<forall>e\<in>E\<^bsub>K\<^esub>. \<^bsub>x \<circ>\<^sub>\<rightarrow> l\<^esub>\<^sub>E e = \<^bsub>y \<circ>\<^sub>\<rightarrow> d\<^esub>\<^sub>E e\<close>
    for D' x y
  proof -
    define u where 
          \<open>u \<equiv> \<lparr>node_map = \<lambda>v. if \<exists>v' \<in> V\<^bsub>D\<^esub>. \<^bsub>p\<^esub>\<^sub>V v' = v then \<^bsub>y\<^esub>\<^sub>V (inv_into V\<^bsub>D\<^esub> \<^bsub>p\<^esub>\<^sub>V v) else \<^bsub>x\<^esub>\<^sub>V (inv_into V\<^bsub>L\<^esub> \<^bsub>g\<^esub>\<^sub>V v), 
                edge_map = \<lambda>e. if \<exists>e' \<in> E\<^bsub>D\<^esub>. \<^bsub>p\<^esub>\<^sub>E e' = e then \<^bsub>y\<^esub>\<^sub>E (inv_into E\<^bsub>D\<^esub> \<^bsub>p\<^esub>\<^sub>E e) else \<^bsub>x\<^esub>\<^sub>E (inv_into E\<^bsub>L\<^esub> \<^bsub>g\<^esub>\<^sub>E e)\<rparr>\<close>
    ...
    proof (rule_tac x = u in exI, rule conjI)
      ...
      show \<open>morphism G D' u 
            ...
            \<and> (\<forall>v\<in>V\<^bsub>D\<^esub>. \<^bsub>u \<circ>\<^sub>\<rightarrow> idM\<^esub>\<^sub>V v = \<^bsub>y\<^esub>\<^sub>V v) \<and> (\<forall>e\<in>E\<^bsub>D\<^esub>. \<^bsub>u \<circ>\<^sub>\<rightarrow> idM\<^esub>\<^sub>E e = \<^bsub>y\<^esub>\<^sub>E e)\<close>
      proof (intro conjI)
       ...
       next
         ...
         proof -
           consider 
               (a) \<open>\<exists>y \<in> E\<^bsub>L\<^esub> - \<^bsub>l\<^esub>\<^sub>E ` E\<^bsub>K\<^esub>. \<^bsub>g\<^esub>\<^sub>E y = e\<close>
           ...
           proof cases
             case a
                obtain y' where \<open>y' \<in> E\<^bsub>L\<^esub> - \<^bsub>l\<^esub>\<^sub>E ` E\<^bsub>K\<^esub>\<close> and \<open>\<^bsub>g\<^esub>\<^sub>E y' = e\<close>
                  using a by blast
  
                have \<open>\<^bsub>u\<^esub>\<^sub>V (s\<^bsub>G\<^esub> e) = \<^bsub>u \<circ>\<^sub>\<rightarrow> g\<^esub>\<^sub>V (s\<^bsub>L\<^esub> y')\<close>
                  using \<open>y' \<in> E\<^bsub>L\<^esub> - \<^bsub>l\<^esub>\<^sub>E ` E\<^bsub>K\<^esub>\<close> and \<open>\<^bsub>g\<^esub>\<^sub>E y' = e\<close> 
                ...
                proof -
                  obtain v' where \<open>s\<^bsub>L\<^esub> y' = v'\<close> and \<open>v' \<in> V\<^bsub>L\<^esub>\<close>
                    using \<open>y' \<in> E\<^bsub>L\<^esub> - \<^bsub>l\<^esub>\<^sub>E ` E\<^bsub>K\<^esub>\<close>
                  ...
                  proof (cases \<open>v' \<in> V\<^bsub>L\<^esub> - \<^bsub>l\<^esub>\<^sub>V ` V\<^bsub>K\<^esub>\<close>)
                    case True
                    then show ?thesis 
                  ...
                  thus ?thesis
                    using 
                      ...
                      \<open>y' \<in> E\<^bsub>L\<^esub> - \<^bsub>l\<^esub>\<^sub>E ` E\<^bsub>K\<^esub>\<close> \<open>\<^bsub>g\<^esub>\<^sub>E y' = e\<close> calculation g.source_preserve 
                    by force
                qed
  
                also have \<open>\<dots> = s\<^bsub>D'\<^esub> (\<^bsub>x\<^esub>\<^sub>E y')\<close>
                  using \<open>y' \<in> E\<^bsub>L\<^esub> - \<^bsub>l\<^esub>\<^sub>E ` E\<^bsub>K\<^esub>\<close> \<open>\<^bsub>g\<^esub>\<^sub>E y' = e\<close>
           ...
           next
             ...
             then show ?thesis
             proof -
               obtain y' where \<open>y' \<in> E\<^bsub>D\<^esub>\<close> and \<open>\<^bsub>p\<^esub>\<^sub>E y' = e\<close>
                 using b by simp
  
               have \<open>\<^bsub>u\<^esub>\<^sub>V (s\<^bsub>G\<^esub> e) = \<^bsub>u \<circ>\<^sub>\<rightarrow> p\<^esub>\<^sub>V (s\<^bsub>D\<^esub> y')\<close>
                 using \<open>y' \<in> E\<^bsub>D\<^esub>\<close> and \<open>\<^bsub>p\<^esub>\<^sub>E y' = e\<close>
               ...
               also have \<open>\<dots> = s\<^bsub>D'\<^esub> (\<^bsub>u\<^esub>\<^sub>E e)\<close>
                 using \<open>y' \<in> E\<^bsub>D\<^esub>\<close> \<open>\<^bsub>p\<^esub>\<^sub>E y' = e\<close>
         ...
         proof -
                ...
                proof -
                  ...
                  thus ?thesis
                    using 
                      ...
                      \<open>y' \<in> E\<^bsub>L\<^esub> - \<^bsub>l\<^esub>\<^sub>E ` E\<^bsub>K\<^esub>\<close> \<open>\<^bsub>g\<^esub>\<^sub>E y' = e\<close> calculation g.target_preserve 
                    by force
                qed
           ...
           next
             ...
             proof -
               ...
               have \<open>\<^bsub>u\<^esub>\<^sub>V (t\<^bsub>G\<^esub> e) = \<^bsub>u \<circ>\<^sub>\<rightarrow> p\<^esub>\<^sub>V (t\<^bsub>D\<^esub> y')\<close>
                 using \<open>y' \<in> E\<^bsub>D\<^esub>\<close> and \<open>\<^bsub>p\<^esub>\<^sub>E y' = e\<close>
                 by (simp add: morph_comp_def)
     ...
     next
        show \<open>l\<^bsub>G\<^esub> v = l\<^bsub>D'\<^esub> (\<^bsub>u\<^esub>\<^sub>V v)\<close> if \<open>v \<in> V\<^bsub>G\<^esub>\<close> for v
         proof -
           consider 
               (a) \<open>\<exists>y \<in> V\<^bsub>L\<^esub> - \<^bsub>l\<^esub>\<^sub>V ` V\<^bsub>K\<^esub>. \<^bsub>g\<^esub>\<^sub>V y = v\<close>
     ...
     next
      ...
      proof - 
        ...
        proof (cases \<open>\<exists>y \<in> V\<^bsub>K\<^esub>. \<^bsub>l\<^esub>\<^sub>V y = v\<close>)
          case True
          then show ?thesis
            using \<open>\<forall>v\<in>V\<^bsub>K\<^esub>. \<^bsub>x \<circ>\<^sub>\<rightarrow> l\<^esub>\<^sub>V v = \<^bsub>y \<circ>\<^sub>\<rightarrow> d\<^esub>\<^sub>V v\<close> 
    ...
    next
      ...
      proof - 
        ...
        proof (cases \<open>\<exists>y \<in> E\<^bsub>K\<^esub>. \<^bsub>l\<^esub>\<^sub>E y = e\<close>)
          case True
          then show ?thesis
            using \<open>\<forall>e\<in>E\<^bsub>K\<^esub>. \<^bsub>x \<circ>\<^sub>\<rightarrow> l\<^esub>\<^sub>E e = \<^bsub>y \<circ>\<^sub>\<rightarrow> d\<^esub>\<^sub>E e\<close> 
            by (auto simp add: u_def morph_comp_def xxx8' that)
        next
          case False
          then show ?thesis
      ...
      qed
    next
      show \<open>\<forall>v\<in>V\<^bsub>D\<^esub>. \<^bsub>u \<circ>\<^sub>\<rightarrow> p\<^esub>\<^sub>V v = \<^bsub>y\<^esub>\<^sub>V v\<close>
        by (simp add: morph_comp_def u_def)
    next
      show \<open>\<forall>e\<in>E\<^bsub>D\<^esub>. \<^bsub>u \<circ>\<^sub>\<rightarrow> p\<^esub>\<^sub>E e = \<^bsub>y\<^esub>\<^sub>E e\<close>
        by (simp add: morph_comp_def u_def)
    qed
  ...
  next
    show \<open>\<forall>ya. morphism G D' ya 
          ...
          \<and> (\<forall>v\<in>V\<^bsub>D\<^esub>. \<^bsub>ya \<circ>\<^sub>\<rightarrow> idM\<^esub>\<^sub>V v = \<^bsub>y\<^esub>\<^sub>V v) \<and> (\<forall>e\<in>E\<^bsub>D\<^esub>. \<^bsub>ya \<circ>\<^sub>\<rightarrow> idM\<^esub>\<^sub>E e = \<^bsub>y\<^esub>\<^sub>E e) 
    ...
    proof safe
      show \<open>\<^bsub>ya\<^esub>\<^sub>E e = \<^bsub>u\<^esub>\<^sub>E e\<close> 
        if \<open>morphism G D' ya\<close> 
          ...
          \<open>\<forall>v\<in>V\<^bsub>D\<^esub>. \<^bsub>ya \<circ>\<^sub>\<rightarrow> idM\<^esub>\<^sub>V v = \<^bsub>y\<^esub>\<^sub>V v\<close> \<open>\<forall>e\<in>E\<^bsub>D\<^esub>. \<^bsub>ya \<circ>\<^sub>\<rightarrow> idM\<^esub>\<^sub>E e = \<^bsub>y\<^esub>\<^sub>E e\<close>
          \<open>e \<in> E\<^bsub>G\<^esub>\<close> for ya e

      proof (cases \<open>e \<in> E\<^bsub>D\<^esub>\<close>)
        case True
        then show ?thesis
          using \<open>\<forall>e\<in>E\<^bsub>D\<^esub>. \<^bsub>ya \<circ>\<^sub>\<rightarrow> idM\<^esub>\<^sub>E e = \<^bsub>y\<^esub>\<^sub>E e\<close>
          by (simp add: u_def)
      next
        case False
        then show ?thesis 
    ...
    next
      ...
      proof (cases \<open>v \<in> V\<^bsub>D\<^esub>\<close>)
        case True
        then show ?thesis
          using \<open>\<forall>v\<in>V\<^bsub>D\<^esub>. \<^bsub>ya \<circ>\<^sub>\<rightarrow> idM\<^esub>\<^sub>V v = \<^bsub>y\<^esub>\<^sub>V v\<close>
          by (simp add: u_def)
      next
        case False
        then show ?thesis 
